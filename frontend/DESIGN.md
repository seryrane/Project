...
- **콘텐츠 max-width 는 두 가지 — 화면 종류가 고른다** (`AppShell` 의 `width` prop, 토큰은
  ...
  글줄은 길었다: **data = 1440**(표·매트릭스·대시보드) ·
  **doc = 960**(가이드·FAQ·개인정보 방침처럼 글이 본체인 화면)
  - ⚠ **1680 → 1440** (2026-08-18 사용자 지적 "본문이 꽉 찬다 · 좌우 폭 축소"). 1680 은
    설계서 본문 폭(1920 − 사이드바 240)이라 **좌우에 남는 것이 패딩 32px 뿐**이었다.
    지금은 좌우로 120px 씩 남고 패딩도 40 이다.
  - ⚠⚠ **지금 세 벌의 상태가 다르다** (2026-08-21 확인):
    | 어디 | 본문 폭 | 카드 안쪽 글줄 |
    | --- | --- | --- |
    | 코드 (`styles.css --container-data`) — **정본** | 1440 | 1360 (1440 − 좌우 40×2) |
    | 산출물 `docs/산출물/HMG_레이아웃설계서.docx` | 1440 ✔ 맞췄다 | 1360 ✔ |
    | Figma 설계서 (`02 화면·웹`, 20장) | **아직 1680** | **아직 1592** |
    Figma 는 20장을 다시 그려야 해서 **일부러 뒤로 미뤘다**(사용자 결정 2026-08-21).
    ⚠ 그때까지 **표 열 폭은 Figma 의 1592 가 아니라 코드의 1360 으로 센다** — 1592 로 짜면
    마지막 열이 화면에서 조용히 잘린다(설계 파일에서 이미 두 번 밟은 자리다).
...
- ⚠⚠ **사이드바도 테마를 탄다** (2026-08-13 사용자 지시로 방침 변경 — 이전에는 "두 테마 모두
  다크 유지, 브랜드 아이덴티티"였다). 밝은 캔버스 옆에 검은 기둥이 서면 **그 기둥만 딴 앱**
  처럼 남는다. 라이트에서는 흰 기둥이 옅은 남색 캔버스 위에 종이처럼 뜬다(헤더와 같은 논리).
  - **사이드바 안에서도 `white/*` 유틸을 쓰지 않는다.** 예전 방침의 부산물로 `white/5`(호버 면)·
    `white/8`(트리 선)·`text-white`(강한 글자)가 마크업에 박혀 있었는데, 기둥이 밝아지는 순간
    **전부 안 보이게 된다**(흰 면 위 흰색 6%). 토큰으로 뺐다:
    `sidebar` · `sidebar-ink` · `sidebar-strong` · `sidebar-hover` · `sidebar-line` · `sidebar-accent`.
  - 예외는 하나 — **활성 항목의 배지**. 그 바탕은 두 테마 모두 보라 그라디언트라 white 가 맞다.
  - 콘텐츠 영역은 여전히 `chip`/`chip-strong`/`hairline` 토큰만 (white/* 금지).
  - 흰 기둥에서는 강조색을 밝힐 이유가 없으므로 라이트의 `sidebar-accent` 는 primary 원색이다.
...
- **선은 세 단계다** — 나누는 선(`divider`, 표·목록의 행) · 감싸는 선(`hairline`, 카드·상자
  테두리 **· 입력칸**) · **버튼의 선**(`control`). 한 값이 여러 일을 하면 카드는 흐리고
  표는 시끄럽고 **버튼은 글자로 읽힌다**.
  - ⚠⚠ `control` 은 2026-08-27 신설. 사용자 지적("버전비교·이력 버튼이 뒷배경으로 인해
    구분이 안 됨")을 재 보니 라이트에서 [임시저장] 테두리가 캔버스 대비 **1.11:1**,
    [버전 비교]는 아예 투명이었다.
  - 값: 다크 `#4b5268` · 라이트 `#a3aec4` — **hairline 의 약 두 배**(실측 1.9~2.6:1).
    ⚠ 같은 날 3:1(WCAG 1.4.11)까지 올렸다가 *"보더라인이 너무 진해지니 촌스러운데?"* 로
    되돌렸다. 고칠 것은 **자국이 0 이었던 것**이지 옅은 것이 아니었다. 3:1 후보값은
    다크 `#616a82` · 라이트 `#7986a3` — 정식 접근성 감사 때 다시 본다.
  - ⚠ **입력칸은 hairline 을 쓴다.** 오목한 면(`--color-field`)과 placeholder 가 이미
    "적는 칸"이라고 말한다 — 여기에 선까지 세우면 화면마다 검색칸이 상자로 튀어나온다.
  - ⚠ **카드·칩·탭·목록 행·아이콘 단추에는 안 두른다**(규약 §23-11). 잣대는 **조작 높이 h-8~11**.
  - 상태색 버튼은 `border-<상태>-ink/45`(오류 입력칸은 `/50` 유지 — 오류는 소리쳐도 된다).
  - ⚠ 2026-08-18 감사: 이 규칙이 **라이트 테마의 `td`·`th` 에만** 걸려 있었다. 다크에서는
    행이 카드 테두리와 같은 무게였고, 목록의 `<li>` 행은 셀이 아니라 규칙 밖이라 화면마다
    `border-hairline/50`·`/60` 으로 **손으로 농도를 깎아** 쓰고 있었다(열 자리).
    지금은 행이 `border-divider` 를 **직접** 쓴다 — CSS 가 멀리서 색을 바꿔치기하지 않는다.
- **호버·깊이는 낮게 잡는다** (2026-08-18 사용자 지적으로 한 단씩 내림):
  카드 리프트 `translate -1px`(예전 3px + 스프링 튕김) · 그림자 shadow-1 은 접지 위주로
  약하게 · 스포트라이트 `420px / 0.18`(예전 320px / 0.35) · 터치 기기(`hover: none`)에서는
  리프트·스포트라이트 **둘 다 끈다**(호버가 없어 마지막 만진 자리에 효과가 남는다).
- **LNB 상위 메뉴(섹션 머리)는 옅은 면 위에 앉는다** — `--color-sidebar-section`(다크 흰 4% ·
  라이트 남색 4%). ⚠ 마크업에서는 **`bg-[var(--color-sidebar-section)]`** 로만 쓴다:
  이름 유틸로 쓰면 Tailwind v4 가 값을 박아 라이트에서 다크 값이 나온다(흰 면에 흰 4% = 무효).
...
- ⚠⚠ **강조색 다섯은 oklch 로 잡는다 — 라이트 `L .45 / C .12`, 다크 `L .74 / C .11`, 색상(H)만 다르다**
  (2026-08-13 "색감톤" 판). 예전에는 손으로 고른 hex 였고, 재 보니 **다섯 다 라이트 캔버스에서
  WCAG AA 미달**이었다(violet 4.06 · blue 4.43 · green 2.83 · amber 2.73 · rose 3.94).
  원인은 하나 — **채도만 높고 명도가 부족했다.** 강조색은 글자로도 쓰이는데(링크·활성 칩·다음 행동)
  밝은 캔버스 위에서 4.5 를 못 넘었고, 같은 이유로 화면에서 "톤이 튄다"는 인상을 줬다.
  **채도를 내리고 명도로 대비를 세우면 두 문제가 같이 풀린다**(고친 뒤 5.92~6.83, 채도는
  0.21 → 0.11 로 브랜드 남색과 같은 대역). L·C 를 눈에 보이는 값으로 직접 잡아야 다섯 색의
  무게가 맞는다 — hex 로는 손대중이 된다(차트 필에서 먼저 배운 것).
  ⚠ green·amber 는 명도를 낮추면 숲색·황동색이 된다. 밝은 초록·주황을 **글자로 쓰면서** AA 를
  넘길 방법은 없다 — 이름이 주는 인상보다 읽히는 것이 먼저다.
  ⚠ 기본 navy(현대 남색)는 브랜드라 그대로 (이미 11.8:1).
