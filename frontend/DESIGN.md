...
- **선은 세 단계다** — 나누는 선(`divider`, 표·목록의 행) · 감싸는 선(`hairline`, 카드·상자
  테두리) · **조작하는 것의 선**(`control`, 버튼·입력칸·셀렉트). 한 값이 여러 일을 하면
  카드는 흐리고 표는 시끄럽고 **버튼은 글자로 읽힌다**.
  - ⚠⚠ `control` 은 2026-08-27 신설. 사용자 지적("버전비교·이력 버튼이 뒷배경으로 인해
    구분이 안 됨")을 재 보니 라이트에서 [임시저장] 테두리가 캔버스 대비 **1.11:1**,
    [버전 비교]는 아예 투명이었다. 조작의 경계는 **WCAG 2.1 1.4.11 · 3:1** 이 기준이다.
    값: 다크 `#616a82`(3.57/3.31/3.07) · 라이트 `#7986a3`(3.13/3.65/3.36) — 캔버스/표면/올린 면.
  - ⚠ **카드·칩·탭·목록 행·아이콘 단추에는 안 두른다**(규약 §23-11). 가르는 잣대는
    **조작 높이 h-8~11** 과 **입력칸 여부**다.
  - 상태색 버튼은 `border-<상태>-ink/70` — 색만으로는 면이 1.2~1.4:1 에 머문다.
