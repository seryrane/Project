...
export const WORK_DICT: Record<string, Entry> = {
  ...
  'board.panel.overlapWarn': { ko: '⚠ 이 사양서에 이미 심사 중인 요청이 {n}건 있습니다 — 신청은 막지 않지만, 반영은 하나로 정리될 때까지 막힙니다.', en: '⚠ {n} request(s) already in review for this spec — submitting is allowed, but deployment is blocked until resolved to one.' },
  'board.panel.withdrawTitle': { ko: '결재 회수', en: 'Withdraw request' },
  'board.panel.withdraw': { ko: '회수', en: 'Withdraw' },
  'board.panel.withdrawHint': { ko: '올린 결재를 거두어 오면 카드는 초안으로 돌아갑니다. 아무도 판단하지 않았을 때만 가능합니다 — 한 단계라도 승인이 찍혔다면 반려를 받아 재요청하는 것이 이력에 남는 길입니다.', en: 'Takes back a submitted request — the card returns to Draft. Possible only before any decision — once a step is approved, get a rejection and resubmit so the history stays honest.' },
  'board.panel.deployTitle': { ko: '배포 요청', en: 'Request deployment' },
  'board.panel.deploy': { ko: '배포 요청', en: 'Request' },
  'board.panel.deployHint': { ko: '배포도 결재를 거칩니다 — 요청이 결재함으로 가고, 배포 승인이 나면 카드가 배포 완료로 옮겨집니다.', en: 'Deployment goes through approval too — the request enters the inbox, and the card moves once deployment is approved.' },
  'board.panel.releaseVersion': { ko: '릴리즈 버전', en: 'Release version' },
  'board.panel.env': { ko: '환경', en: 'Environment' },
  'board.toast.submitted': { ko: '{name} — 결재에 올렸습니다', en: '{name} — submitted for approval' },
  'board.toast.notRequester': { ko: '회수는 요청자만 할 수 있습니다 — 이 건은 {name} 님이 올렸습니다.', en: 'Only the requester can withdraw — this was submitted by {name}.' },
  'board.toast.withdrawn': { ko: '{name} — 회수했습니다', en: '{name} — withdrawn' },
  'board.toast.withdrawBlocked': { ko: '회수할 수 없습니다 — 이미 한 단계 이상 승인이 찍혔습니다. 반려·재요청으로 진행하세요.', en: 'Cannot withdraw — a step has already been approved. Proceed via rejection and resubmission.' },
