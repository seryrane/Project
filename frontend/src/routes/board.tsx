    ...
    return (
      <li>
        <Link
          ...
          className="card-hover block rounded-xl border border-hairline bg-surface p-3.5 transition-colors hover:border-primary/40"
        >
          <div className="flex items-center justify-between gap-2">
            <span className="font-mono text-[11px] text-ink-subtle">{spec.id}</span>
            <span className="font-mono text-[11px] text-ink-subtle">{cur.version}</span>
          </div>
          ...
          <div className="mt-1 text-[11px] text-ink-subtle">{t(`specCategory.${spec.category}`, spec.category)}</div>
    ...
    <AppShell active="board" title={t('nav.board', '상태 보드')}>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 pc:grid-cols-5">
        {COLUMNS.map((st) => {
          ...
          return (
            <section key={st} className="min-w-0">
              <header className="flex items-center gap-2 px-1">
                ...
                <span className="ml-auto rounded-full bg-chip px-2 py-0.5 text-[11px] tabular-nums text-ink-muted">
                  {cards.length}
                </span>
              </header>
              <ol className="mt-2.5 space-y-2.5 rounded-2xl border border-hairline/60 bg-canvas/40 p-2.5">
                {cards.map((sp) => (
                  <Card key={sp.id} spec={sp} />
                ))}
                {cards.length === 0 && (
                  /* 빈 열도 이유를 말한다(§17) — 접으면 흐름의 모양이 사라진다 */
                  <li className="px-2 py-6 text-center text-[11px] text-ink-subtle">
