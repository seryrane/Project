...
#
# ⚠ Devtools 안내 항목은 일부러 지웠다. @tanstack/devtools-vite 는 클라이언트 콘솔을
#   서버로, 서버 로그를 클라이언트로 서로 되쏘아 warn/error 하나가 무한 증폭되는 사고를
#   냈다(dev 서버 4회 사망, 로그 5.4GB). 플러그인·패널·의존성을 모두 걷어냈으므로
#   다시 넣지 않는다 — vite.config.ts 주석과 docs/개발_가이드.md 함정 절 참고.
tanstackIntent:
