...
>
> **이 문서는 사업 배경·원문 자료 위치·확정/미확정의 정본이다.** 역할별로 무엇을 읽을지는
> 저장소 `README.md` 의 문서 지도, 기획 관점 요약은 `docs/기획_가이드.md`.
>
...
- **Workflow**: 신규/수정/삭제 요청별 프로세스, 최대 N단계, 대결, 참조, 반려·재상신, 승인 후 수정 정책. **동일 사양 다중 수정 요청 충돌 관리** — ⚠ 이 항목의 출처는 **요구사항정의서가 아니라 회의록**이다(2026-07-20 41:03~43:20). 2026-08-21 에 회의에서 정해진 범위만 구현했다(신청은 안 막고, 반영만 막고, 하나를 고른 뒤 나머지를 사유와 함께 취소). 남은 정책은 `docs/기획_가이드.md` §8. 최종 반영 전 관리자 승인 필수
